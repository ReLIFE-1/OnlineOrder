<template>
  ...
  >
    ...
    >
      ...
      >
        <template #default="scope">
          <span v-if="scope.row.type != 2" style="font-size: 12px">￥</span
          >{{ scope.row.price }} <span v-if="scope.row.type == 2">积分</span>
        </template>
      </el-table-column>
      <el-table-column
        label="总价"
      ...
      >
        <template #default="scope">
          <span v-if="scope.row.type != 2" style="font-size: 12px">￥</span
          >{{ scope.row.total }} <span v-if="scope.row.type == 2">积分</span>
        </template>
      </el-table-column>
      <el-table-column
        label="地址"
    ...
    context
      ...
      .then((res) => {
        ...
        userinfo.value.money = (
          parseFloat(userinfo.value.money) - parseFloat(row.total)
        ).toFixed(2)
        //如果商品存在库存，则减去商品库存
        if (data.alllimittimes) {
          data.alllimittimes =
            parseInt(data.alllimittimes) - parseInt(row.buynumber)
          //更新商品库存
          context
            ?.$http({
    ...
    context
      ...
      .then((res) => {
        ...
        if (row.type == 2) {
          ...
          context
            ?.$http({
              url: `${context?.$toolUtil.storageGet(
                'frontSessionTable'
              )}/update`,
              method: 'post',
              data: userinfo.value,
          ...
          if (data.jf) {
            userinfo.value.jf =
              parseInt(userinfo.value.jf) - parseInt(row.total)
          }
          // 把减去的余额加回去
          userinfo.value.money = (
            parseFloat(userinfo.value.money) + parseFloat(row.total)
      ...
      tr {
        ...
        td {
          ...
          .cell {
            ...
            // 取消订单
            .cancel_btn {
              border: 0px solid rgba(4, 133, 156, 0.3);
              cursor: pointer;
              border-radius: 4px;
              padding: 0 10px;
              margin: 0 5px 6px 0;
            ...
            // 退款
            .refundPrice_btn {
              border: 0px solid rgba(156, 4, 4, 0.3);
              cursor: pointer;
              border-radius: 4px;
              padding: 0 20px;
              margin: 0 5px 6px 0;
            ...
            // 退货
            .refundGood_btn {
              border: 1px solid rgba(156, 4, 4, 0.3);
              cursor: pointer;
              border-radius: 4px;
              padding: 0 20px;
              margin: 0 10px 6px 0;
            ...
            // 物流
            .logistics_btn {
              border: 0px solid rgba(2, 102, 181, 0.3);
              cursor: pointer;
              border-radius: 4px;
              padding: 0 20px;
              margin: 0 10px 6px 0;
